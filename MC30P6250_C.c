...
#define KEY_DETECT_INIT()   do { DDR13 = 1; P13PU = 0; } while(0)
...
#if RT_DETECT_ENABLE
#define RT_DETECT_INIT()    do { DDR10 = 1; P10PU = 0; P10PD = 1; } while(0)
#define RT_DETECT_ON()      do { P10PU = 0; P10PD = 1; } while(0)   // 检测震动
#define RT_DETECT_OFF()     do { P10PU = 1; P10PD = 0; } while(0)   // 检测充电
...
{
   	T0CR = 0x05; //时钟为CPU时钟    定时器64分频  1ms
    T0CNT = 161;  // (256 - 95);
   	T0IE = 1;
}
/************************************************
...
void key_scan(void)
{
    ...
    if (key_scan_count >= 10)
    {
        ...
        key_filtter <<= 1;
        if (KEY_DET_PIN) key_filtter |= 0x01;
